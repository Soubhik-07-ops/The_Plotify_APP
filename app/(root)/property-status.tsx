      ...
      >
        ...
        <View style={styles.imageContainer}>
          ...
          {item.price && (
            <View style={styles.priceOverlay}>
              <AppText style={styles.priceText}>{formatPriceINR(item.price)}</AppText>
            </View>
          )}
        </View>
        ...
        <View style={styles.cardContent}>
          ...
          <View style={[styles.statusMessageContainer, {
            ...
            <AppText style={[styles.statusMessage, { color: getStatusColor(item.status) }]}>
              {item.status_message}
            </AppText>
          </View>
          ...
          {item.rejection_reason && (
            <View style={styles.rejectionContainer}>
              <AppText style={styles.rejectionTitle}>Rejection Reason:</AppText>
              <AppText style={styles.rejectionText}>{item.rejection_reason}</AppText>
              {item.rejection_category && (
                <AppText style={styles.rejectionCategory}>
          ...
          {item.admin_notes && (
            <View style={styles.adminNotesContainer}>
              <AppText style={styles.adminNotesTitle}>Admin Notes:</AppText>
              <AppText style={styles.adminNotesText}>{item.admin_notes}</AppText>
            </View>
          )}
          ...
          {removalRequest && (
            <View style={styles.removalStatusContainer}>
              ...
              >
                <AppText style={{ fontWeight: '700', color: getRemovalStatusColor(removalRequest.request_status) }}>
                  Removal: {removalRequest.request_status.toUpperCase()}
                </AppText>
              </AppText>
              {removalRequest.admin_notes && (
                <AppText style={styles.removalStatusNotes}>
                  {removalRequest.admin_notes}
                </AppText>
              )}
            </View>
          ...
          <View style={styles.datesContainer}>
            ...
            {item.approved_at && (
              <AppText style={styles.dateText}>
                â€¢ Approved: {new Date(item.approved_at).toLocaleDateString()}
              </AppText>
            )}
            {item.rejected_at && (
              <AppText style={styles.dateText}>
                â€¢ Rejected: {new Date(item.rejected_at).toLocaleDateString()}
              </AppText>
            )}
          </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.background }} edges={['top', 'left', 'right']}>
      ...
      <View style={[styles.header, { backgroundColor: palette.surface, borderBottomWidth: 1, borderBottomColor: palette.border }]}>
        ...
        <AppText style={styles.headerTitle}>My Properties</AppText>
        <View style={{ width: 32 }} />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <AppText style={styles.emptyText}>No properties submitted yet</AppText>
              <AppText style={styles.emptySubtext}>Submit a property to see its status here</AppText>
            </View>
          }
        />
      )}
      ...
      >
        ...
        >
          ...
          >
            <AppText style={{ fontSize: 20, fontWeight: 'bold', color: palette.textPrimary, marginBottom: 16 }}>
              Request Property Removal
            </AppText>

            <AppText style={{ fontSize: 14, color: palette.textMuted, marginBottom: 8 }}>
              Property
            </AppText>
            <View
              style={{
            ...
            >
              <AppText style={{ fontSize: 16, fontWeight: '600', color: palette.textPrimary }}>
                {selectedProperty?.title || '-'}
              </AppText>
              {selectedProperty && (
                <AppText style={{ fontSize: 13, color: palette.textSecondary, marginTop: 4 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              >
                <AppText style={{ color: palette.textPrimary, fontWeight: '600' }}>Cancel</AppText>
              </TouchableOpacity>
              <TouchableOpacity
                style={{
                  flex: 1,
              ...
              >
                {submittingRequest ? (
                  ...
                  <AppText style={{ color: palette.surface, fontWeight: '700' }}>Submit Request</AppText>
                )}
              </TouchableOpacity>
            </View>
          </View>
