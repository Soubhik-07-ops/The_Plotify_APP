...
export type ExplorePropertyFilters = {
  purpose?: 'sale' | 'rent';
  minPrice?: number;
  maxPrice?: number;
  propertyType?: string;
...
export const getProperties = async (
  ...
  limitCount?: number,
  includeSold?: boolean,
  filters?: ExplorePropertyFilters
): Promise<Property[]> => {
  try {
    ...
    if (filters?.propertyType) {
      query = query.contains('categories', [filters.propertyType]);
    }
    ...
    if (filters?.minPrice !== undefined) {
      properties = properties.filter((property) => getNumericPrice(property.price) >= filters.minPrice!);
    }

    if (filters?.maxPrice !== undefined) {
      properties = properties.filter((property) => getNumericPrice(property.price) <= filters.maxPrice!);
    }

    if (filters?.amenities?.length) {
      const selectedAmenities = filters.amenities.map((item) => item.toLowerCase());
      properties = properties.filter((property) => {
        const facilitySet = property.facilities.map((item) => item.toLowerCase());
        return selectedAmenities.every((amenity) => facilitySet.includes(amenity));
      });
    }

    if (filters?.purpose) {
      properties = properties.filter((property) => {
        const tokens = [property.type, property.description, ...property.facilities]
          .join(' ')
          .toLowerCase();

        const isRental =
          tokens.includes('rent') ||
          tokens.includes('rental') ||
          tokens.includes('for rent') ||
          tokens.includes('on rent');

        if (filters.purpose === 'rent') {
          // Keep only clearly rental properties
          return isRental;
        }

        // For "sale" (buy mode), prefer to exclude obvious rentals
